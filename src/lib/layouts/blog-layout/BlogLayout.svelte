<script lang="ts">
	...
	// Exports
	export let blogDetails!: IBlogLayout;
...
{#if blogDetails}
	<article class="flex flex-col justify-center items-start max-w-2xl mx-auto mb-16 w-full">
		...
		<div class="flex flex-col md:flex-row justify-between items-start md:items-center w-full mt-2">
			<div class="flex items-center">
				...
				<p class="text-sm text-gray-700 dark:text-gray-300 ml-2">
					{blogDetails.by}
					{'Lee Robinson / '}
					{format(parseISO(blogDetails.publishedAt), 'MMMM dd, yyyy')}
				</p>
			</div>
			<p class="text-sm text-gray-500 min-w-32 mt-2 md:mt-0">
				{blogDetails.readingTime.text}
			</p>
		</div>
		...
		<div class="text-sm text-gray-700 dark:text-gray-300">
			<ExternalLink href="{discussUrl(blogDetails.slug)}" ariaLabel="{blogDetails.title}">
				{'Discuss on Twitter'}
			</ExternalLink>
			{` • `}
			<ExternalLink href="{editUrl(blogDetails.slug)}" ariaLabel="{blogDetails.title}">
				{'Edit on GitHub'}
			</ExternalLink>
		</div>
	</article>
